...
- Módulo comercial (`/comercial/orcamentos`, `/comercial/vendas`, `/cadastros/clientes`): pipeline cliente → orçamento → aceite por link público sem login → venda → instalação → faturamento → cobrança, com widget de funil no dashboard — ver [comercial.md](comercial.md)
...
- Suíte de testes (`api/tests/`) roda contra o mesmo banco de desenvolvimento (sem `TEST_DATABASE_URL` isolado) — acumula dados a cada execução e colide com o seed fictício de ponto em datas hardcoded (ver [escala-de-trabalho.md](escala-de-trabalho.md#abono-de-ponto-espelho-de-ponto-e-notificações-2026-07-06)). Isso continua sem solução — o que foi corrigido em 2026-08-31 foi um problema diferente: `tests/conftest.py` setava o GUC de RLS com `is_local=true` (`SET LOCAL`), que somia no primeiro `commit()` do meio de uma request e derrubava qualquer teste cujo fluxo fizesse uma query depois de um commit intermediário (ex.: `session.refresh()` após `session.commit()`, padrão comum em todo `service.py` do projeto). Corrigido pra `is_local=false` + `RESET` no teardown, igual `set_tenant_context`/`require_session` em produção — sozinho, isso derrubou a suíte de 123 falhas pra 65 (as 65 restantes são as duplicatas acumuladas do problema de cima, não deste).
