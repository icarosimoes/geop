...
- [ ] **`registro_redis` com 0/1 réplicas em produção desde 2026-07-11 (`Rejected`/"context canceled"), achado incidentalmente durante este deploy — não relacionado a esta mudança.** `/health/ready` já reportava `cache: unavailable` antes de qualquer ação desta sessão. API funciona sem cache (degrada graciosamente), mas dashboard/permissões perdem o benefício do Redis documentado em `mapa.md`. Precisa de investigação (`docker service ps registro_redis --no-trunc`, `docker service logs registro_redis`) e provavelmente só um `docker service update --force registro_redis`.
