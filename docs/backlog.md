...
- [ ] **Identificadores técnicos de infra não renomeados** (decisão explícita do usuário: "vamos manter assim sem mudar os nomes das stack") — banco `registro`, secrets do Swarm, bucket S3 `registro-attachments`, imagens GHCR `registro/*`, nome do stack `registro`. Ficaria para uma janela de manutenção dedicada se algum dia for decidido.
- [ ] **Integração Chess Hotel será descontinuada** ("não teremos mais integração", dito pelo usuário) — nada no código (`X-Registro-Key`, endpoints, `RegistroLauncher.vue`) foi alterado; docs da integração (`chess-hotel.md`/`chess-hotel-implementacao.md`) receberam só parte do rebrand de marca antes do aviso. Sem plano de remoção definido ainda.
