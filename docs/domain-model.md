...
| Núcleo V1 importado | `sectors`, `locations`, `functions`, `procedures` | tenant `aero-hotel` (Aero Hotel), `legacy_id` (nullable — null em registros criados pelo Registro), soft delete e relações remapeadas. `occurrences` foi dropada em 2026-07-14 (fusão em `work_orders`) — deixou de fazer parte do corte V1, que já estava descontinuado |
