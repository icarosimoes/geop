...
**Pendência**: migration `20260713_0061` aplicada em desenvolvimento; **não aplicada em produção ainda** — segue o mesmo processo manual documentado no incidente de deploy anterior (`docker exec ... alembic upgrade head` no manager do Swarm, revisão do conteúdo da migration antes por ser destrutiva — dropa tabela `occurrences` com dados reais de produção, ao contrário das migrations aditivas anteriores). Ver `backlog.md` para o registro desse pendente e da limitação de permissões de papéis customizados.
