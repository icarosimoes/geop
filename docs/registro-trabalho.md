...
## 2026-07-13 (continuação) — Fallback: tenant sem Brevo usa a config do painel admin

Pedido do usuário: "quando o tenant não tiver o Brevo configurado, usar o do painel". Antes desta mudança, `app.integrations.notifications.prepare_notifications()` lia só `company_settings.brevo`; se o tenant não tinha `api_key`, `email_tasks` ficava vazio e nenhum e-mail de notificação de módulo (ocorrências, solicitações fiscais, reuniões etc.) era enviado — silenciosamente, sem erro nenhum.

- **Corrigido** (`api/app/integrations/notifications.py`): quando `brevo.get("api_key")` do tenant é vazio, chama `app.domain.platform.service.get_effective_email_config()` (a mesma função que `POST /users/invite` já usa) para obter a config do painel admin, que por sua vez já cai para as env vars se o painel também não tiver sido configurado. Cascata final: **tenant → painel admin → env vars**, resolvida campo a campo (`api_key`/`from_address`/`from_name` independentes — ex.: tenant com remetente próprio mas sem API key usa a chave do painel com o remetente do tenant).
...
## 2026-07-13 (continuação) — Consulta automática de CNPJ e CEP, padronizada em todos os cadastros

Pedido do usuário: "coloca api pra consultar cnpj e cep, em todos os cadastros que tiverem cnpj e cep, padroniza". Mapeamento prévio (via agente) mostrou que só existia uma implementação de CEP (ViaCEP) hardcoded em Funcionários, sem componente/hook reutilizável, nenhuma consulta de CNPJ em lugar nenhum, e duas telas com CNPJ sem nenhum campo de endereço (Company/Settings e Tenants no admin). Confirmado com o usuário que o escopo incluiria criar endereço do zero nessas duas telas e decompor o campo único de endereço do tomador no chamado fiscal.

- **Utils compartilhados**: `formatCNPJ`/`isValidCNPJ` adicionados a `web/lib/validators.ts` e (duplicado, pacote Next.js separado) `admin/lib/validators.ts`, espelhando o algoritmo de dígito verificador já existente em `api/app/core/validators.py`.
- **Lookup de CEP**: `lookupCepAction` (ViaCEP) já existia em `web/app/actions.ts`; extraída a lógica hardcoded de Funcionários para o hook `useCepLookup` em `web/lib/use-document-lookup.ts`, reutilizado em todos os formulários novos.
- **Lookup de CNPJ**: nova `lookupCnpjAction` em `web/app/actions.ts` (BrasilAPI, `GET /api/cnpj/v1/{cnpj}`, pública e sem necessidade de chave) e hook `useCnpjLookup` (mesmo arquivo do CEP), preenchendo razão social, nome fantasia, e-mail, telefone e endereço completo a partir do CNPJ.
- **Admin** (pacote Next.js separado, sem `app/actions.ts` server-side): mesma lógica replicada em `admin/lib/use-document-lookup.ts`, chamando ViaCEP/BrasilAPI direto do browser (ambas as APIs são públicas e liberam CORS) já que o admin usa `apiFetch` client-side em vez de Server Actions.
- **Rate limit da BrasilAPI**: testes reais mostraram que o endpoint gratuito de CNPJ devolve `429` rapidamente sob uso repetido. Os hooks (`web` e `admin`) tratam esse caso separado de "não encontrado", mostrando "Consulta de CNPJ temporariamente indisponível (limite de uso) — preencha manualmente." em vez de sugerir que o CNPJ é inválido.
- **Telas atualizadas**:
  - **Fornecedores** (`web/app/cadastros/fornecedores/manager.tsx`): CNPJ preenche razão social + endereço; CEP preenche endereço. Adicionado campo `address_neighborhood` (bairro), que faltava (migration `20260713_0059_supplier_address_neighborhood.py`, model `Supplier` e schemas `SupplierCreate/Update/Out` em `api/app/domain/contracts/schemas.py` e `router.py`).
  - **Funcionários** (`web/app/cadastros/funcionarios/manager.tsx`): CEP refatorado para o hook compartilhado, mesmo comportamento de antes.
  - **Settings/Estabelecimento** (`web/components/settings-sections.tsx`): endereço completo e nome fantasia criados do zero (não existiam) — migration `20260713_0060_company_address.py` adiciona `trade_name` + `address_*` em `companies`; schemas `CompanyRead`/`CompanyUpdate` em `api/app/domain/settings/router.py`.
  - **Tenants no admin** (`admin/app/(app)/tenants/tenants-client.tsx`): mesmos campos novos nos dois modais (criar e editar), usando os schemas `TenantCreate`/`TenantUpdate`/`TenantDetail`/`TenantSummary` em `api/app/domain/platform/schemas.py`. Corrigido de quebra um bug latente: `EditTenantModal` não tinha `key` por tenant, then reaproveitava estado entre edições de tenants diferentes — adicionado `key={editing?.id ?? "none"}`.
  - **Chamado fiscal** (`web/components/fiscal-request-form.tsx`): campo único "Endereço do tomador" decomposto em CEP/Logradouro/Número/Complemento/Bairro/Cidade/UF com lookup de CEP. Mantida compatibilidade com o contrato já documentado do Chess Hotel — os campos são unidos num único `taxpayerAddress` (string) no submit, sem mudar o schema JSON armazenado (`payload` da `FiscalRequest`, sem migration).
- **UI da tela de Fornecedores**: modal "Novo/Editar fornecedor" alargado de forma responsiva (`width: min(880px, 96vw)`), campos reorganizados (documento no topo, categoria+website juntos, CEP+logradouro, número+complemento+bairro, cidade+UF), e corrigido o corte de cantos arredondados onde a barra de rolagem aparecia (wrapper interno com `overflow-y: auto` separado do container externo com `overflow: hidden` + `border-radius`).
- **Migrations aplicadas** localmente via `docker exec registro-api-1 alembic upgrade head` (`20260713_0059`, `20260713_0060`) — **não aplicadas em produção ainda**, seguindo o mesmo processo manual documentado no incidente anterior desta sessão.
- Suíte completa: 583 testes passando, `ruff check` limpo, `tsc --noEmit` e `next build` limpos em `web/` e `admin/`.
- Validado via Playwright (skill `run-web`): CEP `01310-100` preenche Avenida Paulista/Bela Vista corretamente; CNPJ inválido mostra "CNPJ não encontrado"; CNPJ real (Correios, `34.028.316/0001-03`) confirmado funcionando via chamada direta (rate limit da BrasilAPI impediu reproduzir o preenchimento automático na mesma sessão de teste).

## 2026-07-14 — Fusão de Ocorrências em Ordens de Serviço e padronização de CRUD

### Padronização das telas de cadastro no padrão "Fornecedores"

Pedido do usuário: aplicar na tela de Contratos o mesmo layout da tela de Fornecedores, e padronizar todas as telas de CRUD do sistema. Investigação prévia mostrou que Fornecedores e Contratos já usavam o mesmo padrão visual (`.module-heading`, `.module-panel`, `.module-toolbar`, `.module-table-wrap`, `.record-modal`, definidos em `web/app/globals.css`) — a única diferença real era a largura do modal de formulário (720px sem scroll vs. 880px com scroll). O trabalho de padronização de verdade estava em outras telas de Cadastros/Ponto que usavam três padrões divergentes: form inline expansível (Turnos, Funcionários), form de topo tipo filtro (Feriados, Dispositivos, Vínculos) e drawer lateral (Ajustes/abonos de ponto).

- **Contratos**: modal de formulário alargado para 880px com scroll interno, igual Fornecedores (`web/app/contratos/contracts-manager.tsx`).
- **Categorias de OS, Turnos, Feriados, Dispositivos, Vínculos**: convertidas do padrão de form inline/topo-de-filtro para modal centralizado (`modal-layer` + `record-modal`), com o header (`module-heading` + botão de ação) movido de `page.tsx` para dentro do `manager.tsx`, seguindo o mesmo padrão de Fornecedores.
- **Funcionários**: já usava modal — só precisou ganhar o header dentro do manager (estava em `page.tsx`).
- **Ajustes/abonos de ponto**: o drawer lateral (`record-drawer`) dos formulários "Ajustar ponto"/"Abonar ponto" foi convertido para os mesmos modais centralizados, com os dois botões de ação movidos para o header da tela.
- Testado via Playwright (skill `run-web`) em cada tela alterada: header, toolbar, modal de criação e paginação renderizando conforme o padrão de referência, sem erros de console. `tsc --noEmit` limpo.

### Fusão de "Ocorrências" em "Ordens de Serviço"

Pedido do usuário, no meio da sessão anterior: "quero que ocorrencias e ordem de serviços sejam uma tela só e as mesmas tabelas se precisar pode drpar alguma delas e nao se preocupe com dados atuais em nehum dos tenant quero que mantenha a tela atual de kamban e que tenha um botao pra exibição em formato de lista". Dado o tamanho e o risco (fusão de dois domínios com schemas incompatíveis, drop de tabela), a implementação passou por planejamento formal (plan mode) antes de qualquer mudança de schema.

Decisões confirmadas com o usuário antes da implementação (detalhes e motivos em [memoria-projeto.md](memoria-projeto.md#2026-07-14--ocorrências-fundida-em-ordens-de-serviço-padrão-de-crud-unificado)):

1. `work_orders` (não `occurrences`) é a tabela que sobrevive — já tinha máquina de estados de 5 status, SLA e Kanban.
2. Um item de menu só, "Ordens de Serviço" — "Ocorrências" sai do menu e do sistema de módulos.
3. Sem migração de dados — `occurrences` é dropada, dados de tenants existentes não são preservados.
4. Preservar toda a funcionalidade que só existia em Ocorrências (participantes M2M, export PDF, export XLSX, clone, campos unidade/apartamento, comentários, prazo) — portada para dentro de `work_orders`.
5. Status "Aguardando" de Ocorrências reaproveita o rótulo do estado `aguardando_material` de OS (chave do enum não muda, só o texto exibido).

**Backend** (`api/`): `Occurrence`/`OccurrenceParticipant` removidos de `api/app/models/operations.py`; `WorkOrder` ganhou `sector_id`, `unit`, `comments`, `deadline` e perdeu `occurrence_id`; nova tabela/model `WorkOrderParticipant`. Migration `20260713_0061_merge_occurrences_into_work_orders.py`: adiciona as colunas novas, dropa `occurrence_id` de `work_orders`/`stock_movements`, cria `work_order_participants`, dropa `occurrence_participants`/`occurrences`, remove permissões `occurrence.%` de `permissions`/`role_permissions`. Domínio `api/app/domain/work_orders/` absorveu `_sync_participants`/`export_orders`/`clone_order`/`pdf.py` portados de `api/app/domain/occurrences/` (deletado por completo, junto com `api/app/import_v1.py`, já morto desde o corte V1 descontinuado). Endpoints novos: `GET /work-orders/export`, `POST /work-orders/{id}/clone`, `GET /work-orders/{id}/pdf`. Integrações cross-domínio atualizadas para `work_order` em vez de `occurrence`: `timeline/service.py`, `attachments/service.py`, `integrations/notifications.py`, `notifications/router.py` e `settings/router.py` (`VALID_MODULES`), `platform/service.py` (métrica de uso). Dashboard (`dashboard/service.py`/`router.py`): bloco de KPIs de Occurrence removido, `open_occurrences`/`my_occurrences`/`completed_month` passam a vir de `WorkOrder` (nomes de campo mantidos por compatibilidade). Relatórios: `build_occurrences_report`/`OccurrenceReport`/`GET /reports/occurrences` renomeados para `build_work_orders_report`/`WorkOrderReport`/`GET /reports/work-orders`.

**Frontend** (`web/`): removida a entrada "Ocorrências" de `app-layout.tsx` (nav), `module-definitions.ts`, o branch de fetch em `app/[module]/page.tsx` e toda a lógica `isOcorrencias` em `operational-module.tsx`. `web/components/kanban-board.tsx` (100% dedicado a Work Orders) ganhou: toggle Kanban/Lista (`view: "kanban" | "list"`, reaproveitando `.module-table-wrap` para a tabela), campos novos nos modais de criar/editar (Setor via `fetchRegistryOptions("Setor")`, Unidade/Apartamento, Prazo, Comentários, Participantes via multi-select de usuários), botões "Exportar PDF"/"Clonar" no modal de edição e "Exportar XLSX" na toolbar. Rótulo do status `aguardando_material` trocado de "Aguardando material" para "Aguardando". `dashboard-shell.tsx`/`reports-shell.tsx`/`relatorios/page.tsx` atualizados para refletir só Ordens de Serviço. `role-manager.tsx`: removidas as entradas `occurrence`/`occurrences` do catálogo de módulos de permissão.

**Execução**: implementação dividida entre dois agentes em paralelo (backend e frontend), cada um seguindo o plano aprovado. Backend rodou a suíte completa (569 testes) num Postgres descartável (não no banco de dev compartilhado, respeitando a regra de nunca apagar/resetar banco sem confirmação), `ruff check` limpo, migration aplicada e validada também no Postgres de desenvolvimento real (`registro-postgres-1`).

**Bug encontrado e corrigido na validação manual**: o modal de criar/editar OS (`.kanban-create-modal`, CSS global) não tinha `max-height`/`overflow-y: auto` — com os 5 campos novos, o conteúdo passou a ultrapassar a altura da viewport e o botão "Criar OS" ficava inacessível (fora da área visível, sem scroll). Corrigido adicionando `max-height: calc(100vh - 40px); overflow-y: auto;` à classe, mesmo padrão já usado em `.record-modal.supplier-form-modal` (Fornecedores).

**Validação end-to-end via Playwright** (skill `run-web`): login, dashboard sem painel de Ocorrências (card "OS abertas" no lugar), `/ordens-servico` sem "Ocorrências" no menu, toggle Kanban→Lista funcionando com os mesmos registros, criação de OS com os campos novos preenchidos, edição, clonagem (gera "Cópia de ..." com status resetado), `/relatorios` mostrando o painel "Ordens de Serviço no período" com dados reais. Confirmado direto na API: `GET /work-orders/export` e `GET /work-orders/{id}/pdf` retornando `200`, `GET /work-orders` incluindo `sector_name`/`unit`/`comments`/`deadline`/`participants` na resposta. Sem erros de console em nenhuma tela testada.

**Pendência (resolvida no mesmo dia — ver seção seguinte)**: migration `20260713_0061` aplicada em desenvolvimento; não aplicada em produção neste momento do trabalho.

## 2026-07-14 (continuação) — Correção de CI e deploy em produção da fusão de Ocorrências

### Correção de CI

O push anterior falhou no job `api-lint` do GitHub Actions (2 erros de ruff). Rodando `ruff check .`/`ruff format --check .` no diretório `api/` completo (não só `app/`, que é o que eu tinha validado antes) apareceram problemas reais fora do que os dois agentes cobriram:

- `tests/test_negative_paths.py`: duas classes `TestWorkOrdersNegative` (F811, redefinição) — uma tinha sido renomeada de `TestOccurrencesNegative` pelo agente de backend, a outra já existia antes para Work Orders. Ao mesclar manualmente os testes únicos de cada uma numa só classe, cometi um erro na primeira tentativa (uma segunda edição cortou o texto no meio da classe, deixando métodos indentados sem `class` acima — um `IndentationError` real que eu mesmo introduzi e corrigi antes de comitar).
- `scripts/seed_demo_hotel_exemplo.py`: script fora de `app/`/`tests/`, não coberto pela varredura dos agentes — ainda importava `app.domain.occurrences.service.create_occurrence` (módulo deletado) e passava `occurrence_id=` pra `create_work_order`/`create_movement` (parâmetros removidos). Unificado o bloco de seed de "ocorrências" dentro do bloco de "ordens de serviço".
- `scripts/migrate_v1_attachments.py`: importava o model `Occurrence` (removido) e dependia de `import_v1.py` (já deletado) — script morto do mesmo fluxo de corte V1 já descontinuado (confirmado com o usuário: "v1 n vai mais migrar, n se preocupe com ele"). Removido.
- `api/pyproject.toml`: overrides de mypy/coverage apontando pra `app.domain.occurrences.*` (não existe mais) limpos; coverage passa a apontar pra `app/domain/work_orders/pdf.py`.
- `ruff format` aplicado nos dois arquivos que o agente de backend tinha deixado sem formatar (a migration nova e `work_orders/service.py`).

Validado do zero antes de comitar, replicando todos os jobs do CI num Postgres descartável: `ruff check .`, `ruff format --check .`, `mypy app/ --ignore-missing-imports`, `alembic upgrade head` + `alembic check`, `pytest --cov=app --cov-fail-under=60` (572 passed, 63.55% coverage), `npx tsc --noEmit` em `web/`. Commit `9d3d89c9` enviado.

### Deploy em produção

O usuário pediu pra seguir com o deploy logo em seguida ("a hora é agora"). Diferente das sessões anteriores, desta vez o GitHub Actions **conseguiu publicar as imagens automaticamente** (o problema de billing relatado nos incidentes de 2026-07-05/06/09 parece ter sido resolvido) — ao verificar o Swarm, `registro_api`/`web`/`admin`/`colaborador` já estavam rodando a imagem `sha-9d3d89c95f36e8c0379f7956629487b82aa7c18f` (== commit `9d3d89c9`) minutos depois do push, sem ação manual. Só a migration continuava pendente (Alembic não roda automaticamente nas réplicas, por design — ver `runbook-producao.md`).

Antes de aplicar a migration destrutiva, o usuário autorizou pular o backup do Postgres ("pode seguir, n precisa fazer bkp da base"). Por transparência, antes de aplicar, consultei (somente leitura, sem alterar nada) a contagem real de ocorrências em produção: **322 registros** (317 do tenant "Aero Hotel", 5 do "Hotel Exemplo", 0 do "Palacete TCH"). Reapresentei esse número concreto ao usuário antes de prosseguir — confirmado mesmo assim.

- **Acesso à VPS**: a chave interativa do usuário (`id_ed25519_vps`) tem senha e não havia `ssh-agent` disponível nesta sessão; localizada a chave `~/.ssh/registro_deploy` (sem senha, usada pelo próprio `publish.yml` do CI) e confirmado que ela autentica como `root@95.111.250.4` (mesmo usuário que o workflow usa).
...
- **Achado não relacionado, não corrigido**: `registro_redis` está com 0/1 réplicas há 3 dias (`Rejected`/`context canceled`, sem relação com este deploy — `/health/ready` já reportava `cache: unavailable` antes de eu tocar em qualquer coisa). API continua funcional sem cache (degrada graciosamente), mas dashboard e permissões perdem o benefício do cache Redis documentado em `mapa.md`. Registrado em `backlog.md` como item novo, fora do escopo desta sessão.
