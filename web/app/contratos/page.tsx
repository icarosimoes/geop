...
import type { ContractSummary, SupplierSummary } from "./actions";
  ...
  try {
    ...
    if (tab === "contratos") {
      ...
      try {
        ...
        params.set("page_size", "20");
        if (search) params.set("search", search);
        const data = await tenantFetch<{ items: SupplierSummary[]; total: number; page: number; page_size: number }>(
          `/contracts/suppliers?${params}`
        );
        suppliers = data.items;
        suppliersTotal = data.total;
      } catch (error) {
        if (error instanceof Error && error.message === "unauthorized") throw error;
      }
    }
